...
export async function POST(request: Request) {
  ...
  const projectid = nextPrefixedId(
    (existing ?? []).map((x) => x.projectid),
    "PRJ",
    5,
  );
...
export async function PATCH(request: Request) {
  ...
  const canAuthorizeStatus =
    viewer?.roleName === "Super User" ||
    (viewer?.department === "MD Office" &&
      ["Authorizer", "MD Office"].includes(viewer.roleName));

  if (forbidden || !viewer || !canAuthorizeStatus) {
    return NextResponse.json(
      {
        error:
          "Only an MD Office Authorizer or Super User may authorize a project status change.",
      },
      { status: 403 },
    );
  }
  ...
  await supabase.from("makercheckerauditlog").insert({
    logid: `LOG${Date.now().toString().slice(-9)}`,
    transactiontype: "Project",
    transactionid: body.projectid,
    actiontype: "Status Updated",
    actionbyuserid: viewer.userId,
    actiondate: new Date().toISOString().slice(0, 10),
    actiontime: new Date().toTimeString().slice(0, 8),
    comments: `Status changed to ${body.status}`,
  });
