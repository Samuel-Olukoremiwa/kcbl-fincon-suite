...
export async function POST(request: Request) {
  ...
  if (forbidden || !viewer) {
    return NextResponse.json({ error: `Not authorized to create ${kind}s.` }, { status: 403 });
  }
  if (viewer.roleName === "Authorizer") {
    return NextResponse.json({ error: `Authorizers cannot create new ${kind}s.` }, { status: 403 });
  }
  ...
  const { error } =
    kind === "supplier"
      ? await supabase.from("suppliers").insert({
          supplierid: id, suppliername: body.name, supplycategory: body.specialty, ...base,
        })
      : await supabase.from("subcontractors").insert({
          subcontractorid: id, subcontractorname: body.name, tradespecialty: body.specialty, ...base,
        });
