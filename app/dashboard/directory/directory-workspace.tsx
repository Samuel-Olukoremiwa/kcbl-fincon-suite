...
export default function DirectoryWorkspace({
  ...
  const [type, setType] = useState("All client types");
  const statuses = ["Ongoing", "Completed", "On Hold", "Pending"];
  const visibleClients = useMemo(
    () =>
      clients.filter((client) => {
        const projectsForClient = projects.filter(
          (project) => project.clientid === client.clientid,
        );
        const matchesQuery =
          `${client.clientid} ${client.fullnameorcompanyname} ${client.email} ${projectsForClient.map((p) => `${p.projectid} ${p.projecttitle}`).join(" ")}`
            .toLowerCase()
            .includes(query.toLowerCase());
        return (
          matchesQuery &&
          (type === "All client types" || client.clienttype === type) &&
          (status === "All statuses" ||
            projectsForClient.some((project) => project.status === status))
        );
      }),
    [clients, projects, query, status, type],
  );
  const types = [
    ...new Set(clients.map((client) => client.clienttype).filter(Boolean)),
  ];
  return (
    <>
      ...
      <div className="mt-6 space-y-6">
        {visibleClients.map((client) => {
          ...
          return (
            <section className="card overflow-hidden" key={client.clientid}>
              ...
              <div className="divide-y divide-slate-100">
                {clientProjects.map((project) => (
                  <ProjectCard
                    ...
                    project={project}
                    managerName={
                      managerNames[project.projectmanageruserid] ??
                      "Not assigned"
                    }
    ...
    <article className="p-5">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div className="min-w-0">
          <p className="font-semibold text-ink">{project.projecttitle}</p>
      ...
      <div className="mt-5 grid gap-4 md:grid-cols-[1fr_1fr]">
        <div className="grid grid-cols-2 gap-x-6 gap-y-4 text-sm">
          ...
          <Detail label="Expected end" value={date(project.expectedenddate)} />
        </div>
        {!canViewFinancial ? (
          ...
          <div className="rounded-md border border-navy-100 bg-navy-50/40 p-4">
            ...
            <div className="mt-3 grid grid-cols-3 gap-3">
              ...
              <Amount
                ...
                tone={inflow - outflow < 0 ? "text-red-700" : "text-navy"}
              />
            </div>
          </div>
        )}
...
function Detail({
  ...
  return (
    <div>
      ...
      <p
        className={`mt-1 break-words text-sm text-ink ${restricted ? "select-none blur-[1px]" : ""}`}
      >
        {value}
      </p>
    </div>
...
function Amount({
  ...
  return (
    <div>
      ...
      <p className={`table-number mt-1 text-sm font-semibold ${tone}`}>
        {value}
      </p>
    </div>
  );
