...
type NavGroup = { label: string; items: (NavItem & { module: ModuleKey })[] };
...
export default function Sidebar({
  ...
  const groups = (isClient ? clientGroups : staffGroups)
    .map((group) => ({
      ...group,
      items: group.items.filter((item) => canAccess(viewer, item.module)),
    }))
  ...
  return (
    <>
      <button
        ...
        className="fixed left-4 top-3 z-40 rounded-md bg-navy p-2 text-white shadow md:hidden"
      ...
      <aside
        className={
          ...
          "fixed inset-y-0 left-0 z-40 flex w-64 flex-col bg-navy-900 transition-transform md:translate-x-0"
        }
      >
            ...
            <div key={group.label}>
              ...
              {group.items.map((item) => {
                ...
                return (
                  ...
                  >
                    <Icon size={18} strokeWidth={1.75} className={active ? "text-amber" : ""} />
                    {item.label}
                  </Link>
                );
              })}
