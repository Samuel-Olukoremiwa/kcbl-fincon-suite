...
export default function ProjectWorkspace({
  ...
  async function updateStatus(projectid: string, nextStatus: string) {
    ...
    setSaving(null);
    if (!res.ok)
      return setMessage(data.error ?? "Could not update project status.");
    router.refresh();
  }
  ...
  return (
    <div className="mt-8">
      <div className="flex flex-wrap gap-3 border-b border-slate-200 pb-4">
        <button
          className={view === "existing" ? "btn-primary" : "btn-secondary"}
      ...
      {view === "new" && canCreate ? (
        <div className="card mt-6 max-w-xl p-6">
          <ProjectForm
            existingIds={projects.map((p) => p.projectid)}
            clients={clients}
            managers={managers}
          />
        </div>
        ...
        <div className="mt-6 space-y-4">
          {!canViewFinancial && (
            <p className="rounded-md bg-slate-100 px-4 py-3 text-sm text-slate-600">
              Financial values are restricted for your department.
            </p>
          )}
          ...
          {clients.map((client) => {
            const rows = visible.filter((p) => p.clientid === client.clientid);
            if (!rows.length) return null;
            return (
              <section className="card overflow-hidden" key={client.clientid}>
                <div className="border-b border-slate-200 bg-slate-50 px-4 py-3">
                  <b>{client.fullnameorcompanyname}</b>
                  <span className="ml-2 text-xs text-slate-400">
                    {client.clientid} · {rows.length} project(s)
                  </span>
                </div>
                        ...
                        return (
                          <tr key={project.projectid}>
                            <td className="px-4 py-3">
                              ...
                              <span className="block text-xs text-slate-400">
                                {project.projectid} · {project.projectlocation}
                              </span>
                            </td>
                            <td className="px-4 py-3">
                              {canViewFinancial ? (
                                money(project.estimatedvalue)
                              ) : (
                                <RestrictedValue />
                              )}
                            </td>
                            ...
                            {!canViewFinancial ? (
                              ...
                              >
                                Financial records are hidden for completed
                                projects.
                              </td>
                            ) : (
                              <>
                                <td className="px-4 py-3 text-green-700">
                                  {money(project.inflow)}
                                </td>
                                <td className="px-4 py-3 text-red-700">
                                  {money(project.outflow)}
                                </td>
                                <td
                                  className={`px-4 py-3 font-medium ${
                                    project.inflow - project.outflow < 0
                                      ? "text-red-700"
                                      : "text-ink"
                                  }`}
                                >
