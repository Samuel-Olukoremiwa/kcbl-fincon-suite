...
import { Users, FileCheck2, Banknote, Building2, Truck, HardHat, Landmark, WalletCards, ReceiptText, CircleDollarSign, Clock3, ArrowRight } from "lucide-react";
...
export default async function DashboardPage() {
  ...
  const [{ data: projects }, { data: inflows }, { data: outflows }] = await Promise.all([
    supabase.from("projects").select("projectid, projecttitle, estimatedvalue, status, startdate, expectedenddate").order("projectid", { ascending: false }),
    ...
    <div>
      ...
      <section className="mt-8">
        ...
        <div className="mt-4 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          <Metric icon={CircleDollarSign} label="Total project value" value={money(totalBudget)} sub={`${projects?.length ?? 0} project(s)`}/>
          ...
          <Metric icon={Clock3} label="Current cash position" value={money(cashPosition)} sub={`${pending} pending approval(s)`}/>
        </div>
      </section>

      <section className="mt-8 grid gap-5 xl:grid-cols-[1.35fr_1fr]">
        <div className="card overflow-hidden"><div className="flex items-center justify-between border-b border-slate-200 p-5"><div><h2 className="font-semibold text-ink">Project tracker</h2><p className="mt-1 text-sm text-slate-500">Current project values and progress dates.</p></div><Link href="/dashboard/projects" className="text-sm font-medium text-navy">View projects</Link></div><div className="divide-y divide-slate-100">{projects?.slice(0, 5).map(project => <div key={project.projectid} className="flex items-center justify-between gap-4 p-5"><div><p className="font-medium text-ink">{project.projecttitle}</p><p className="mt-1 text-xs text-slate-400">{project.projectid} · {date(project.startdate)} to {date(project.expectedenddate)}</p></div><div className="text-right"><p className="font-medium">{money(project.estimatedvalue)}</p><span className="mt-1 inline-block rounded-full bg-navy-50 px-2 py-0.5 text-xs text-navy">{project.status}</span></div></div>)}{!projects?.length && <p className="p-10 text-center text-slate-400">Create a client and project to begin live tracking.</p>}</div></div>
      ...
      <section className="mt-9"><p className="section-title">Workspace</p><p className="mt-1 text-sm text-slate-500">Open a module to manage its records.</p><div className="mt-4 grid grid-cols-[repeat(auto-fit,minmax(280px,1fr))] gap-5">
        ...
        <DashboardCard
          ...
          enabled={true}
        />
        <DashboardCard href="/dashboard/suppliers" icon={Truck} title="Suppliers" description="Supplier records and payment details." enabled={true}/>
        <DashboardCard href="/dashboard/subcontractors" icon={HardHat} title="Subcontractors" description="Trade partners and bank details." enabled={true}/>
...
function Metric({ icon: Icon, label, value, sub }: { icon: React.ElementType; label: string; value: string; sub: string }) { return <div className="card p-5"><div className="flex h-10 w-10 items-center justify-center rounded-md bg-navy text-white"><Icon size={20}/></div><p className="mt-4 text-sm text-slate-500">{label}</p><p className="mt-1 text-xl font-semibold text-ink">{value}</p><p className="mt-1 text-xs text-slate-400">{sub}</p></div>; }
...
function DashboardCard({
  ...
  return (
    <Link href={href} className="card group p-5 transition-all hover:-translate-y-0.5 hover:shadow-md">
      {content}
    </Link>
  );
}
